...
export function EmployeeDashboardPage() {
    ...
    const { user } = useAuth()
    const { data: tasks, isLoading } = useTasksByAssignee(user?.uid || null)
    ...
    const stats = {
        ...
        overdue: tasks?.filter((t) => {
            ...
            return due < new Date() && t.status !== TaskStatusConst.COMPLETE
        }).length || 0,
    }
                            ...
                            <div className="space-y-3">
                                {tasks.map((task) => {
                                    ...
                                    const isOverdue =
                                        ...
                                        task.status !== TaskStatusConst.COMPLETE
                                        ...
                                        >
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="text-sm text-slate-500 truncate">
                                                    {client?.name} • Due {new Date(task.dueDate).toLocaleDateString()}
                                                </p>
                                            </div>
